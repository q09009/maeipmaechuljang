...
설정 화면에서는 SQLite와 PostgreSQL 사이의 전체 데이터 이전을 실행할 수 있습니다. 이전 전 양쪽 데이터베이스를 백업하며, 거래처·품목·전표 건수와 내용 해시가 일치할 때만 성공으로 처리합니다. 대상 데이터베이스의 기존 내용은 병합되지 않고 교체됩니다.

레거시 SQLite의 빈 숫자 값은 `0`으로 정규화하며, 올바르지 않은 날짜가 있으면 해당 전표 ID를 알려주고 대상 데이터베이스를 변경하지 않습니다.
